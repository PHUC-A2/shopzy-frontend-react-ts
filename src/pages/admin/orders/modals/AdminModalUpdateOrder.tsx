...
import type { IOrder, IUpdateOrderReq } from '../../../../types/backend';
import { updateOrder } from '../../../../config/Api';
import { useEffect } from 'react';

interface IProps {
    openAdminModalUpdateOrder: boolean;
    setOpenAdminModalUpdateOrder: (v: boolean) => void;
    fetchAllOrders: () => void;
    orderUpdate: IOrder | null;
}

const AdminModalUpdateOrder = (props: IProps) => {
    const { openAdminModalUpdateOrder, setOpenAdminModalUpdateOrder, fetchAllOrders, orderUpdate } = props;
    const [form] = useForm();

    const handleUpdateOrder = async (data: IUpdateOrderReq) => {
        try {
            const res = await updateOrder(data);
            if (res?.data?.statusCode === 200) {
                await fetchAllOrders();
                setOpenAdminModalUpdateOrder(false);
    ...
    useEffect(() => {
        if (openAdminModalUpdateOrder && orderUpdate) {
            form.setFieldsValue(orderUpdate);
        }
    }, [openAdminModalUpdateOrder, orderUpdate]);
    ...
    return (
        <>
            <Modal
                ...
                open={openAdminModalUpdateOrder}
                onOk={() => form.submit()}
                okText="Save"
                maskClosable={false}
                onCancel={() => setOpenAdminModalUpdateOrder(false)}
                width={1000}
            >
                <Form
                    form={form}
...
export default AdminModalUpdateOrder;